...
\begin{itemize}
    \item \textbf{Data drift}: Data drift occurs when the data used during inference in production exhibits significant differences compared to the data used during training, that is,  $P_{train}(X) \neq P_{inference}(X)$.
    \item \textbf{Concept drift}: Concept drift is evoked when a change in the statistical relationship between the features ($X$) and the target variable ($Y$) is observed over time, that is, $P_{train}(Y|X) \neq P_{inference}(Y|X)$ and $P_{train}(X) = P_{inference}(X)$.
\end{itemize}
...
\subsubsection{How to implement a monoting system in the SSP Cloud?}
